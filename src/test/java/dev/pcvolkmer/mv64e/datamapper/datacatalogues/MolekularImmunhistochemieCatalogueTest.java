...
class MolekularImmunhistochemieCatalogueTest {
  ...
  void shouldUseCorrectQuery(@Mock Map<String, Object> resultSet) {
    ...
    assertThat(captor.getValue())
        .isEqualTo(
            "SELECT patient.patienten_id, dk_molekularimmunhisto.*, prozedur.patient_id, prozedur.hauptprozedur_id FROM dk_molekularimmunhisto JOIN prozedur ON (prozedur.id = dk_molekularimmunhisto.id) JOIN patient ON (patient.id = prozedur.patient_id) WHERE geloescht = 0 AND prozedur.id = ?");
  }
  ...
  void shouldUseCorrectMerkmalQuery(@Mock Map<String, Object> resultSet) {
    ...
    assertThat(captor.getValue())
        .isEqualTo(
            "SELECT feldname, feldwert FROM dk_molekularimmunhisto_merkmale WHERE eintrag_id = ?");
  }
