...
class FollowUpCatalogueTest {
  ...
  void setUp(@Mock JdbcTemplate jdbcTemplate) {
    ...
    this.catalogue = FollowUpCatalogue.create(jdbcTemplate);
  }
  ...
  void shouldUseCorrectQuery(@Mock Map<String, Object> resultSet) {
    ...
    assertThat(captor.getValue())
        .isEqualTo(
            "SELECT patient.patienten_id, dk_dnpm_followup.*, prozedur.patient_id, prozedur.hauptprozedur_id FROM dk_dnpm_followup JOIN prozedur ON (prozedur.id = dk_dnpm_followup.id) JOIN patient ON (patient.id = prozedur.patient_id) WHERE geloescht = 0 AND prozedur.id = ?");
  }
