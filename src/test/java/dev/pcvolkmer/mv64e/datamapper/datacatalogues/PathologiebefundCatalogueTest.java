...
class PathologiebefundCatalogueTest {
  ...
  void setUp(@Mock JdbcTemplate jdbcTemplate) {
    ...
    this.catalogue = PathologiebefundCatalogue.create(jdbcTemplate);
  }
  ...
  void shouldUseCorrectQuery(@Mock Map<String, Object> resultSet) {
    ...
    assertThat(captor.getValue())
        .isEqualTo(
            "SELECT patient.patienten_id, dk_pathologie.*, prozedur.patient_id, prozedur.hauptprozedur_id FROM dk_pathologie JOIN prozedur ON (prozedur.id = dk_pathologie.id) JOIN patient ON (patient.id = prozedur.patient_id) WHERE geloescht = 0 AND prozedur.id = ?");
  }
  ...
  void shouldUseCorrectMerkmalQuery(@Mock Map<String, Object> resultSet) {
    ...
    assertThat(captor.getValue())
        .isEqualTo("SELECT feldname, feldwert FROM dk_pathologie_merkmale WHERE eintrag_id = ?");
  }
